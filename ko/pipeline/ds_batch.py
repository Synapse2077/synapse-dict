#!/usr/bin/env python3
"""DeepSeek 的跑批器（ko 专用拷贝）。接口与 it/ja 两份完全一致。2026-09-21。

🔴 **拷 ja 那份不拷 it 那份**，理由是实测的差别不是印象：
   it 那份**没有 `is_peak()` / `announce_window()`** —— 跑批前不报现在是不是高峰价。
   那个机制 de 建好过、ja 传到了、**it 至今没有**
...
import paths as _ko_paths            # noqa: E402
ENV_PATH = _ko_paths.ENV
