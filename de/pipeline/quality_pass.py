#!/usr/bin/env python3
"""de 服务级质检 —— 全部走 2.1pro（量不大，不用 lite 路由）。

三个子命令：
  --fill-ipa    给 ipa 为空的 lemma（7303 条）补德语 IPA，直接写库。
...
IPA_SYS = """你是德语语音专家。我给你一批词（可能含外来词、地名、缩写、复合词、古拼写等），逐个给标准德语 IPA。
约定：标主重音 ˈ；区分元音长短(长音加 ː，如 Tag /taːk/、Buch /buːx/；短音不加，如 mit /mɪt/)；ch→ç(前元音后)或x(后元音后)；sch→ʃ；sp/st 词首→ʃp/ʃt；词尾清化(-d→t/-b→p/-g→k)；-er 词尾→ɐ；ä→ɛ、ö→ø/œ、ü→y/ʏ；r→ʁ；两侧加斜杠，例 /ˈʃøːnhaɪt/。
缩写、纯数字序号、无单词读音的 → 该项返回 null。
严格输出 JSON，键与输入序号一致：{"1":"/.../","2":null,...}，无多余文字。"""

QA_SYS = """你是资深德语→简体中文词典审校专家。我给你一批已翻词条，每条含：词形 w、词性 pos、英文 gloss（消歧参考，可能有误）、以及待审的中文译文数组 zh（每项对应一个义项）。
请**独立**用你的德语知识逐义项核对 zh 是否准确、地道、无错义/漏译/多译/词性错配。英文 gloss 只作参考，以德语实际含义为准。
对每个词返回：
