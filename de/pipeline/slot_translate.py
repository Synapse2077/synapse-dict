...
def done_keys(out_path, land="de"):
    """→ {落盘键: 记录}。坏行（末行截断等）跳过。
...
def translate(items, sys_prompt, out_path, fields=("de", "ctx"), keep=("de", "kind", "n"),
              key_field="de", answer_field="zh", land="de", think=False):
    ...
    **裁决族传 `m`** —— 那一族的产物不是中文，是一张「哪条法语原文挂到哪条义项」的表；
    ...
    stat = asyncio.run(_run(todo, env()["DEEPSEEK_API_KEY"].strip(),
                            ...
                            key_field=key_field, answer_field=answer_field, think=think))
