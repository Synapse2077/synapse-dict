#!/usr/bin/env python3
"""阶段 0 前置清洗：搭配行的**分隔符**归一，让迁移 100% 可逆。2026-08-31（de）。

═══ 为什么需要这一步 ═══
阶段 0 的可逆性回核（`build_v3_schema.py` 闸①）逮到 **1 行**：

    原: 'vor Ort  在现场'      ← 分隔处两个空格
    建: 'vor Ort 在现场'       ← 重建成一个

`split_colloc` 两边都 `.strip()`、重建时用单空格拼回 ⇒ 这 1 行搬进新表后**搬不回来**。
16,783 行里只有这 1 行。

⭐ **pt 那轮同一处逮到 2 行** —— 同一个形状第二次，而这次是闸自己在落库当天报出来的，
   不是事后发现的。这就是"可逆性回核必须 100% 非抽样"的兑现：1/16,783 抽样抽不到。

═══ 为什么不是"记账放着" ═══
可逆性回核的**全部价值**在于它是 100%、非抽样 —— 它把「义项和释义有没有配错」
从抽样问题变成**可判定**问题（`[[verification-gates-not-sampling]]`，用户 2026-08-07：
「不要把义项和释义错配了，那才是真灾难」）。
留 1 行不可逆，这道闸就从"可判定"降级成"99.99%"，以后每次报红都要先想"是不是那 1 行"。

═══ 为什么不是"加一列存原始分隔符" ═══
为 1 行给 `collocation` 表加一列是过度设计，而且那一列此后永远要维护。
...
⚠️ 本步**只改分隔符不改内容**：拆出来的德语部分和中文部分逐字节不变，
   变的只有它们之间的空白。改完 `collocation` 列的非空行数不变（15,393），
...
def roundtrip(ln):
    ...
    a, b = split_colloc(ln)
    if a is None:                       # 切不出德语部分：原样搬，本来就可逆
        return ln
