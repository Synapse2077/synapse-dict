#!/usr/bin/env python3
"""修 B 组（语义存疑 flag，非 misalign，467 条）：pro online 逐词审校+修正。
...
SYS = """你是资深德语→简体中文词典审校专家。给你一批词条（这些是标记为"存疑"的词），每条含：词形 w、词性 pos、英文 gloss（消歧参考，可能有误或过窄）、当前中文译文数组 zh。
请**独立**用你的德语知识逐义项核对 zh 是否准确地道、有无错义/漏译/多译/词性错配。英文 gloss 仅参考，以德语实际含义为准。
对每个词返回：
