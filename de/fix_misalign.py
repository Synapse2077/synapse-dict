...
SYS = """你是德语→简体中文词典翻译专家。给你一批德语词条，每条含词形 w、词性 pos、英文释义数组 gloss（每个元素是一个义项）。
逐义项翻译成地道简体中文，返回 "zh" 数组：
- **长度必须严格等于 gloss 数组长度，一一对应**，绝不合并、拆分、增删义项。
- 英文释义仅作参考，以德语实际含义为准（英文可能有误或过窄）。
- 一个义项内多个近义中文用"，"分隔；只给中文释义本身，不加词性/性别等标注。
严格输出 JSON，键与输入一致：{"1":{"zh":["义项1","义项2",...]},...}，无多余文字。"""

CHECK_SYS = """你是资深德语→简体中文词典审校专家。给你一批词条，含词形 w、英文 gloss 数组、中文译文 zh 数组。
