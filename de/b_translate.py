...
8. 返回 "ipa" 字段：该词音位式国际音标，德语约定——长元音加 ː（gehen /ˈɡeːən/）、词首元音可带声门塞音 ʔ、小舌音 ʁ、ch 按前后作 ç/x、词尾清化（Tag /taːk/、Hund /hʊnt/），两侧加斜杠。无读音的省略 ipa。
9. 返回 "col" 字段：该词最常用搭配或固定短语 1-3 条，形如 "zu Hause 在家"（德语在前、中文在后，空格分隔）。只给搭配/短语，不要整句例句。无则 []。
10. 若你强烈认为缺了常见义项，在 flag 写"疑缺义:<简述>"，但不要为此往 zh 加义项（zh 数量必须严格等于 senses）。
11. 生僻/拿不准/可能歧义的词，flag 写简短原因；有把握则 flag=null。
12. 返回 "level" 字段：该词的 CEFR 难度等级，取值 A1/A2/B1/B2/C1/C2 之一（A1 最基础常用、C2 最生僻高阶）；按该德语词的实际使用频率与掌握难度判断，务必每词都给。
13. 严格输出 JSON，键与输入一致，格式 {"1":{"zh":[...],"gender":"n","genitive":"Hauses","plural":"Häuser","aux":"sein","praeteritum":"ging","partizip2":"gegangen","separable":true,"sep_prefix":"an","comparative":"...","superlative":"...","ipa":"/.../","col":["..."],"level":"B1","flag":null},...}（各本质字段无则省略，level 必给），不要多余文字。"""
