...
ACCEPT = {
    ...
    "D6 有义项的词形没有读音（读者口径）": (
        75_754,  # 🔴 2026-09-18 再从 66,456 涨到 75,754：C45（`translate_foreign_defs.py`）
                 #    又给 9,623 个空白页补上中文释义，同一个机制、同样补不了音标。
                 # 🔴 2026-09-18 从 34,606 涨到 66,456，**涨的 31,850 是预料之中的**：
                 #    C44（`backfill_blank_from_zh.py`）给 31,852 个原本**空白**的词形补上了
                 #    中文释义 —— 它们从「没有义项」变成「有义项但没有音标」，于是掉进这个桶。
                 #    **不是回归，是分母变了**：在此之前它们连「有义项的词形」都不算。
                 #    ⚠️ 这一桶补不了：de 已定不造 G2P（C5），而这批词形**德语版根本没有条目**
                 #    （正是它们当初空白的原因）⇒ 外版也没有音标可抄。
                 #    🔴 上调基线必须写清多出来的是什么，否则它就是个掩盖回归的开关。
                 # 🔴🔴 2026-09-05 判据口径改成「读者看得见的」⇒ 18,124 → 47,453，
        ...
        "75,754 个（收尾单 C21）。**其中 31,850 来自 C44、9,298 来自 C45**，都是补中文释义带来的新增（见上）；余下 34,606 分三块，逐块对得上账："
    ...
    "B5 空白页（无义项、无变形、无指针）": (
        8_204,
        # 🔴 2026-09-18 二次收紧 17,505 → 8,204（收尾单 C45）：下面②那 10,600 条
        #    决定翻了，`translate_foreign_defs.py` 落库 9,623 条义项 / 9,623 个页面，
        #    实测账单 940,851 token ≈ 2.29 元（空闲半价）。
        # 🔴🔴 **2026-09-18 从 49,364 收紧到 17,505，而且旧理由是错的，一起改掉。**
        # 旧理由写着「①源头**两版** `forms` 里根本没有归属 45,119」——
        # 账按 de+en 两版算，**词却是从七版收的**（阶段 3：de 736,097／en 60,537／
        # zh 32,073／fr 20,690／pt 1,902／it 1,354／es 505）。
        # 中文版那 32,073 个词形我们**收了词头、没收释义**，于是一个我们自己造成的
        # 缺口顶着「源头没有」的名义躺了半个月，直到用户点开
        # `Freunde in der Not gehen hundert auf ein Lot.` 报「没有详情页」。
        # `[[dont-say-source-lacks-what-we-skipped]]`：「源头没写」和「我们没抽」
        # 被混成了一个数 —— **这正是那条记忆说的、结构上必须分开的两件事**。
        # ⇒ `de/fixes/backfill_blank_from_zh.py` 补了 31,852 个页面，
        #   另 7 个是句末标点重复的空壳，并到已有的好页面上。
        "8,204 / 1,202,926 ＝ 0.68%。**逐块对得上账**：\n"
        "①哪一版都没有 6,728（五版并集覆盖 86.4%，剩下的 13.6% 是真的哪一版都没有）；\n"
        "②~~只有法/意/西/葡语释义 10,600~~ **已翻（C45）**，其中 1,352 条"
        "模型判定给不出、诚实留空 —— 收紧「不许生造音译」之后它宁可不给，**缺不是错**；\n"
        "③中文版给的其实是英文/德语 71 条 —— `lang` 该标 en 还是 de 分不出来，"
        "**标错语言是错、丢掉只是缺**（`FRAMEWORK §一`）；\n"
        "④清洗后无内容可写 106（整条只有 `〈阴〉`、只有 `{{{1}}}`、"
        "或整条是「X的变格形式」这种指针 —— 指针该走 `inflection` 不走 `sense`）。\n"
        "🔴 **推翻它需要**：出现新的免费中文来源，或那 1,352 条留空的决定换个模型再问一遍。\n"
        "—— 以下为 2026-09-18 之前的旧账，留档 ——\n"
    ...
    "L1 展示层还没接 v3 的表": (
        2,
        # 🔴 2026-09-18 从 3 收紧到 2：**离开名单的是 `sense_src`**，而且不是有意接的 ——
        #    2026-09-14 的词源层给义项算 `etymKey` 时要从 `sense_src.src_ref` 取 5 段式键，
        #    于是 `german.ts` 里出现了 `FROM sense_src`。判据（出现在 FROM/JOIN 后面）
        #    如实反映了这件事，**而基线从那天起松了四天没人收** —— 正是这条断言
        #    自己在下面骂的第四种机制。这次是闸报「⬇ 该收紧」我才来改的。
        #    ⚠️ 读者仍然看不到证据层的内容：那句 SQL 取的是 `src_ref`（一个内部键），
        #      不是 `sense_src.text`。**判据盯的是「有没有查这张表」，不是「查了哪一列」** ——
        #      要盯后者得另写一条，本轮不写，先把数字和理由对上。
        "2 张，**阶段 8 之后 13 → 2**：`entry`、`field_src`，两张都是**有意不接**。\n"
        "      · `field_src` 是**一等字段的证据层**（哪些值没有源头背书），与 `sense_src` 同理："
        "读者该看的是值本身，不是它的出处。**它存在就是为了给我们审计用的**。\n"
        "      · ~~`sense_src`~~ 2026-09-18 离开名单（词源层要读它的 `src_ref`，见上）。\n"
