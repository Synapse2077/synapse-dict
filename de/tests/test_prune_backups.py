#!/usr/bin/env python3
"""`dbtool.prune_backups` 的测试。2026-08-31 随 de 阶段 -2 建（拷自 pt，本体语种无关）。

🔴 **为什么每个语种都要有自己的一份**：这个函数会自动删数据，而它按 `DB.stem` 分语种
   glob 自己的备份 —— 换句话说每门语言各跑各的淘汰。没有本语种的行为测试，
   就等于「pt 的测试绿着，de 的删除逻辑没人验过」。
...
④ 本语种在 `docs/<X>_PLAN.md` 里标了「✅ 完结」之后，预算收到 `DONE_BACKUP_BYTES`
   （2026-09-01 加）：阶段里程碑的意义是「万一这一步做错了退回来」，
...
def run(specs, keep_plain=None, max_bytes=None, done=False, done_bytes=None):
    ...
    🔴 `done` **默认 False，而且必须显式钉死**（2026-09-01）：
       规则④上线那天，`_language_is_done()` 读的是真的 `docs/PT_PLAN.md`，
       而那天正好写上了「✅ pt 完结」⇒ **上面每一条用例的预算都会被悄悄换成
       `DONE_BACKUP_BYTES`**，`max_bytes=BIG` 那几条从此测的不是它们声称在测的东西。
       用例本身还是绿的 —— 这正是最坏的一种坏法。
...
CASES = [
    ...
    (
        # 🔴 规则④（2026-09-01）：**阶段全做完之后，那条楼梯就没有意义了。**
        #    现场是 pt —— 十一个 v3 阶段里程碑 7.9 GB，旧规则试算「删 0 个」，
        #    因为它刚好卡在 8 GB 下面 1%。这里造同一个形状：预算宽松时一个不删，
        #    标了完结之后收到只剩**最老（整轮重构之前，不可再生）**
        #    和**最新（回滚上一步）**。
