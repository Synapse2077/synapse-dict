# de/ 目录约定（2026-08-01 分层）

**根目录只放可被 `import` 的模块**，子目录放可执行脚本。
分层前 16 个 .py 全平铺在一起，"当前有效的"和"跑完就是历史的"看不出来。
