#!/usr/bin/env python3
"""de 工具的金标准测试 —— 把 2026-08-01 翻过的车逐条固化成断言。
...
跑：  python3 -m unittest test_tools -v      （在 de/ 目录下）
"""
import sqlite3
import unittest
...
class 解析(unittest.TestCase):
    ...
    def test_德语版方括号(self):
        self.assertEqual(K.parse_ipa("[ˈvasɐ]"), "ˈvasɐ")

    def test_记法变体两个都要返回(self):
        """fr 版德语 12.3% 多读音，是记法差异（r vs ʁ、次重音）不是方言。
        取值时要选与本库约定一致的 ʁ —— 前提是先拿到全部变体。"""
        e = {"sounds": [{"ipa": "\\ˈfraɪ̯taːk\\"}, {"ipa": "\\ˈfʁaɪ̯ˌtaːk\\"}]}
        v = [ip for ip, _ in K.sounds_variants(e)]
        self.assertEqual(len(v), 2)
        self.assertTrue(any("ʁ" in x for x in v), "选 ʁ 的前提是它没被丢掉")
