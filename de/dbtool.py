#!/usr/bin/env python3
"""德语写库闸门 —— 备份 / 保留策略 / 不变量核对 / 抽样反验。de 专用，不 import 其他语种。

═══ 为什么有这个文件（2026-08-01 立，2026-08-31 按 fr/pt 两轮的教训重做）═══
本文件是 `pt/dbtool.py` 的**拷贝+改语种特有部分**，不是 import ——
铁律①「按本质设计，按语种解耦，互不引用；每个语种一套脚本，宁可重复不要耦合」。

七月那版 192 行，`TRACK` 只有 6 列（ipa/translation/gender/pos/infl/level）——
德语最本质的那批字段**全在闸外面**：`genitive`/`plural`（名词两大著录形）、
`praeteritum`/`partizip2`/`aux`/`vclass`（动词三基本形式+助动词+强弱）、
`separable`/`sep_prefix`（可分动词）—— 恰恰是 `docs/lang/de-DESIGN.md` 列为
"一等字段"的那批，闸却看不见它们变没变。fr 与 pt 两轮犯的是同一个错，
**这次是照着它们的修法一次做对，不是重新发现一遍**。

⚠️ `PLAYBOOK` 1.4：「不能推后。es 是先修了一周数据才有闸门的，那一周的写库全部无法追溯。」
   de 的结构重构一次写库动几十万行，闸门必须在**第一次写库之前**就位 ——
...
用法：
    ...
    with dbtool.session("keep-v3-schema", expect={...}) as s: ...
...
TRACK = ['ipa', 'ipa_src', 'pos', 'level',
         # 名词本质：三性 + 属格单数 + 复数 + 多性名词的逐性别变格束
         'gender', 'gender_src', 'genitive', 'plural', 'noun_variants',
         # 动词本质：三基本形式 + 完成时助动词 + 强弱 + 可分 + 反身 + 支配
         'praeteritum', 'partizip2', 'aux', 'vclass',
         'separable', 'sep_prefix', 'reflexive', 'government',
         # 形容词本质
...
# 判据是**计划表里那一行**，不是手工开关：
#   `docs/DE_PLAN.md` 里出现 `# ✅ pt 完结` 才算完结。
...
PLAN_DOC = paths.ROOT / "docs" / "DE_PLAN.md"
...
def _language_is_done():
    ...
    删数据的默认值必须偏保守。"""
    try:
        return bool(re.search(r"^#+\s*✅\s*de\s*完结", PLAN_DOC.read_text("utf-8"), re.M))
    except OSError:
        return False
