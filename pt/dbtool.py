#!/usr/bin/env python3
"""葡萄牙语写库闸门 —— 备份 / 不变量核对 / 抽样反验。pt 专用，不 import 其他语种。
...
用法：
    ...
    plan = [(new_val, rid), ...]
    with dbtool.session("ipa-fill", expect={"ipa_br": +196368}) as s:
        s.executemany("UPDATE dict SET ipa_br=? WHERE id=?", plan)
...
TRACK = ['ipa_br', 'ipa_pt', 'translation', 'gender', 'pos', 'infl', 'level']
