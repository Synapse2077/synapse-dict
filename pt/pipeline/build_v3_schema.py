#!/usr/bin/env python3
"""阶段 0：葡语库 v3 结构迁移 —— 建十一张表，把 `dict` 的内容列搬进去。2026-08-29。

结构照 `docs/SCHEMA.md`，**不重新设计**。计划见 `docs/PT_PLAN.md` 阶段 0。
本文件是 `fr/pipeline/build_v3_schema.py` 的葡语版（拷贝+改语种特有部分，铁律①按语种解耦），
差异全部写在下面「pt 与 fr 的五处不同」——**逐条量过，不假设同构**。

═══ 这一步到底在干什么 ═══
`dict` 现在把「一个词的所有义项」粘在一个字符串里，用 \n 分行：

    definition  = "book / pound (unit of weight) …"    （英文，104,009 行义项）
    translation = "书 / 磅（重量单位） …"                 （中文，逐行对齐）
...
═══ 迁移前已全量核对（2026-08-29，非抽样）═══
    definition 行 104,009 == translation 行 104,009 == meta 元素 104,009    **零错位**
    definition / translation 内空行各 0 条
    meta 的键只有 pos / g / lex(9,396) / reg(7,600)，lex/reg 全是数组
      ⇒ `sense_tag` 的 (kind,value) 主键不丢信息

═══ pt 与 fr 的五处不同（照搬前逐条量过）═══

① **变形层判据仍是 `translation == infl`**，pt 的比例同样极端：
   无 definition 的 340,382 行里，**340,362 行**的 translation 与 infl 逐字节相同
   （模板生成的 "piar 的 虚拟式现在时第一/三人称单数"），可安全丢弃 —— `infl` 列留着。

② 🔴 **剩 20 行不是变形**：`custodiar`（看管，保管，监护）、`escrevivência`
   （边缘群体书写自我生存体验）、`droga recreativa`（娱乐性毒品）—— 无英文释义、
   `infl` 为 NULL，但中文是**真释义**，且全部 `is_lemma=1`。
   按"无 definition 即变形层"迁移会把这 20 条释义直接丢掉。它们照常建 `sense`。
   ⭐ **fr 58 行 / it 22 行 / pt 20 行 —— 三轮都有，这是第三分支不是特例。**

③ 🔴 **5,927 行同时有 `definition` 和 `infl`** —— 词既是词头又是变形（fr 那轮 4,818）：
   `livre` = 书/自由的 + livrar 的变位、`abate` = 屠宰 + abater 的变位。
   它们走分支①建义项，`infl` 列**本步一个字不动**，留给阶段 2 建 `inflection` 表。

④ **葡语一等字段本步不动**（`vconj`/`pp`/`pp_short`/`pronominal`/`feminine`/`plural`/
   `adj_pos`/`government`/`comparative`）。它们归 `entry`（整词级），而 `entry` 是**阶段 1** 的活。
   本步只搬义项，不碰词条层 —— 一次只动一样东西（`[[one-problem-at-a-time]]`）。
   ⚠️ `comparative` 只有 6 行、`pp_short` 只有 62 行，删列与否是阶段 1 的决定，**这里不删**
   （`[[prefer-reversible-designs]]`：不删就永远可逆）。

⑤ 🔴🔴 **`pronunciation` 表一开始就带 `pos` 列 —— 这是 pt 比 fr 早走的一步。**
   fr 是做到阶段 8 之后的**族 D 评审**才发现"读音要按词性归位"（`taper` 页头摆着名词的
   /te.poer/），补 `pos` 列是返工。pt 在**阶段 -1 就量出来了**：同一词形不同词性读音不同的
   有 **484 个**，规律是葡语的**元音交替（metaphony）**——名词闭元音 /o e/ 对动词开元音 /开o 开e/
   （`acordo` 名 /a-KOR-du/ 动 /a-KOR(开)-du/），**系统性的形态音位对立，压平会抹掉词义**。
   ⇒ 一次建对，不返工（`PLAYBOOK` 开篇：顺序错了要返工，而返工最贵）。
   ⚠️ 同理 `region` 用 `pt-BR`/`pt-PT`，**null = 通用或判不出** ——
      法语版给的 179,722 条音标 99% 不标地区，它们落 null，不许硬塞进任何一边。
...
═══ 🔴 `--drop-cols` 本阶段**先不要跑** ═══
`packages/dict-core/src/portuguese.ts` 是老单表版，阶段 8 才重写。
**现在降列会立刻打断葡语页面，一直断到阶段 8。**
...
DDL = [
    ...
    """CREATE TABLE sense (
         ...
         pos     TEXT,                    -- 逐义项词性（实测 27.9% 的多义词逐义项不同，fr 25.1%）
         gender  TEXT,                    -- 逐义项性别（实测 5.4% 逐义项不同，fr 4.8%）
         UNIQUE(word_id, rank)
       )""",
    ...
    """CREATE TABLE pronunciation (
         ...
         region     TEXT,                 -- pt-BR / pt-PT；null = 通用或**判不出**
                                          --   🔴 法语版 179,722 条 99% 不标地区 ⇒ 落 null，不硬塞
         tags       TEXT,                 -- 源头 tags/raw_tags 的 JSON 数组
                                          --   巴西内部方言（Carioca/Paulistana…）存这里
         pos        TEXT,                 -- 🔴 这条读音属于哪个词性；null = 整词通用
                                          --   484 个词形名/动读音不同（metaphony），见文件头 ⑤
         is_primary INTEGER NOT NULL DEFAULT 0,
         src        TEXT NOT NULL,        -- en-edition / fr-edition / pt-edition / de-edition
...
def split_colloc(ln):
    """→ (葡语, 中文|None)。判据 = **第一个汉字之前的最后一个空格**（`SCHEMA` §7.1）。
    不是"第一个汉字处"——那样 `pH 值` / `do majeur C大调` 这类会切错，
    而跨语言的那个符号本来就属于中文那半。es 上旧判据 99.48%，fr 上 13,980/13,984；
    pt 的实际切分率由试算时打印，**不预设**。"""
    m = CJK.search(ln)
    if not m:
        return ln, None
    ...
    for did, pos0, d, t, m, infl in con.execute(
        ...
        if dl:                                        # ① 有英文释义的词条
            ...
            for i, en in enumerate(dl):
                ...
                #    实测 meta 的 pos 键 104,009 == 义项数 ⇒ 兜底其实一次都不会触发。
...
def snapshot_orig(con):
    ...
    for rid, d, t, m, c in con.execute(
        ...
        #    它的可逆性由 `infl` 列保证（迁移前已全量核对：340,362 行逐字节相同）。
        if t is not None:
...
def main():
    ...
    if a.verify or a.mutate:
        ...
        #    fr 那轮的实况：阶段 2a 往 `sense` 补了 5,011 条 alt_of 义项并重排 rank ⇒
        ...
        if "entry" in have and not a.db:
            ...
            #    只写 `*pre-keep-v3-entry*` 会把别的语种一起列出来，
            #    照着跑就是拿另一门语言的库验这一门。（fr 2026-08-22 打印出来才看见。）
            ...
            if not baks:
                ...
                print("         fr 2026-08-22 就发生过：备份保留策略把那个锚点挤掉了。")
                print("         保留策略后来改成了字节封顶 + 按稀疏度抽稀，首尾永不删。")
            return 0
        cols = {r[1] for r in con.execute("PRAGMA table_info(dict)")}
