...
═══ 为什么必须排在收词（3b）之前 ═══
3b 要从葡语版吃进 跨版并集 64.3 万词形、**保留精确大小写**。
如果先收词：`Brasil` 会作为新行插进来拿到葡语版义项，
而英文版给的「苏格兰」义项还留在折叠行 `brasil` 上 —— **同一个词裂成两行、各拿一半**。
⇒ 先拆干净，再收词。
...
def apply_(con, todo, w_of, sense_of, infl_of, n_new):
    ...
    for wid, keep, others, keep_eids in todo:
        ...
        #    不重算的话 `brasil` 拆完仍写着 `name/v`（苏格兰的 name 已经搬走了）。
...
def verify(con):
    ...
    checks = [
        ...
        # 🔴 2026-08-29 删掉了三条：`dict 行数` / `义项总数不变` / `变形总数不变`。
        #    ① 它们写死的是 **fr 的数字**（388,992 / 124,766 / 331,377），移植到 pt
        #       当场三条全红而数据完全正确 —— 写死行数的断言必然过期，跨语种更是必然错。
        #    ② 更要紧的是**它们本来就是多余的**：`dbtool.session(expect=…)` 已经在
        #       写库那一刻核对过 `__rows__` / `#sense` / `#inflection` 的增量，
        #       而且期望值是调用方**当场算出来**的、不是字面量。
        #    ⇒ 用字面量再查一遍，只是多了一个会过期的东西。职责归 dbtool。
        #    本函数只留**自足的不变量** —— 不需要"写库前的值"就能判定对错的那些。
        ("rank 不从 1 连续的词形",
