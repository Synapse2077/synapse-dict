...
def done_keys(out_path, land="pt"):
    """→ {落盘键: 记录}。坏行（末行截断等）跳过。

    `land` = 落盘按哪个字段存，见 `translate()` 里那一大段。默认 `pt`。
    """
    p = Path(out_path)
    if not p.exists():
        return {}
...
async def _ask(cl, key, sys_prompt, items, fields, think=False, key_field="pt",
               answer_field="zh"):
    ...
    return out, (u.get("total_tokens", 0),
                 u.get("prompt_tokens", 0), u.get("completion_tokens", 0))


async def _run(todo, key, sys_prompt, out_path, fields, keep, key_field="pt",
               answer_field="zh"):
    import httpx
        ...
        async def worker():
            while True:
                try:
                    ...
                    return
                try:
                    got, tok3 = await _ask(cl, key, sys_prompt, ch, fields,
                                          key_field=key_field,
...
def translate(items, sys_prompt, out_path, fields=("pt", "ctx"), keep=("pt", "kind", "n"),
              key_field="pt", answer_field="zh", land="pt"):
    """items = [{fr, …}]。只请求尚未落盘的。→ stat dict。

    `fields` = 发给模型的字段；`keep` = 落盘时保留的字段（答案字段自动加）。

    `key_field` = **应答按哪个字段认领**。默认 `pt`（短槽值，原串就是天然主键）。
    整句释义要用 `id` —— 葡语原文太长，让模型原样回传一整句既费 token 又容易被它
    "顺手改一个字"导致认领不上。

    `land` = **落盘按哪个字段存**，默认 `pt`（`keep` 里必须有它）。
    默认这条是**翻译族的不变量**：同一句葡语在全库永远只有一个中文，所以按原串
    落盘既能续跑又能天然去重。

    🔴 **判断族不能用这条不变量。** 判官的产物取决于 `(法语, 中文)` 这一**对**，
       而同一句葡语配不同中文是存在的 —— 族 E 探针实测：98,305 条义项里有
