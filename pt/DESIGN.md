...
故不照搬 it/fr 的 `aux`（按本质「该删就删」）。

**2026-07-21 规则审计后补齐三缺口**（跑全量豆包翻译前一次到位，承 fr 审计做法）：
- `pp_short`：particípio duplo 不规则短形（ganhar→ganho、pagar→pago、gastar→gasto、
  aceitar→aceito、prender→preso），与 ser/estar 连用、现代葡语常作默认。kaikki forms 有
  `short-form` tag，build.py 确定性抽（零豆包）。原 `pp` 存规则长形（ganhado）。
- `adj_pos`：形容词位置 pre/post/both，位置变义（velho amigo 老友 / amigo velho 年长的朋友）——
  与 fr/es/it 同性质，此前误判为"可后续"，实为一等本质。豆包填。
- `government`：动词/形容词介词支配 regência（gostar de、precisar de、assistir a、consistir em）——
  与 fr `government` 同。豆包填。
