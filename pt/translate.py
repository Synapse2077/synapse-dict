"""
葡萄牙语 lemma 中文释义：豆包批量补 translation 为空的词条。

变位形式在 build.py 已生成中文（translation 非空），不会被选中——
所以这里只翻真正的 lemma，量按 lemma 算（约 54 万）。

流程：读 synapse-dict-pt.sqlite 里 translation 为空的词 → 豆包批翻
     → 结果追加写 doubao-pt.jsonl（断点续传）→ 跑完 merge 回库。
...
def build_prompt(batch: list) -> str:
    ...
    return f"""你是专业的葡萄牙语-中文词典编辑。请为以下葡萄牙语单词提供简洁准确的中文释义。

要求：
- 每个词释义控制在 50 字以内
- 格式为"词性. 释义"，多个词性用换行分隔，如 "n. 房子\\nv. 居住"
- 附有英文释义参考时，请据此翻译
- 返回严格的 JSON 对象，key 为葡萄牙语单词（保持原始大小写），value 为中文释义字符串
- 只返回 JSON，不要任何其他内容
