#!/usr/bin/env python3
"""日语写库闸门 —— 备份 / 保留策略 / 不变量核对 / 抽样反验。ja 专用，不 import 其他语种。
...
⚠️ `PLAYBOOK` 1.4：「不能推后。es 是先修了一周数据才有闸门的，那一周的写库全部无法追溯。」
   所以这是日语的第一个代码文件，排在任何数据动作前面 —— 现在库还不存在。

═══ 🔴 最关键的一条：未声明的列必须零变化 ═══
`expect` 里没写的列，写库前后非空计数**必须完全相同**，否则报错退出。
`expect` 是**增量**不是总数。

═══ 🔴🔴 日语专属：`has_han` 这个名字在本文件里**故意不存在** ═══
另外六门的 dbtool 都导出 `has_han()`，而调用方普遍拿它当「**这是不是中文**」用
（`translate_defs.py` 的控制组、`ingest_*_senses.py` 的分语言落桶、各门的回归闸，
共 82 处）。那条等价关系在拉丁字母语言上成立，**在日语上恒真**：

    has_han('日本語') → True      has_han('食べる') → True      has_han('猫') → True

于是「模型有没有把日语译成中文」这道闸，问的是一个**恒真命题**：
模型把 `日本語` 原样抄回来，闸照样绿。一条永远通过的检查等于没有检查。

⇒ 本文件把它拆成三个，并且**不提供 `has_han` 这个名字**：

    has_han_char(s)   纯粹的字符问题：这串里有没有汉字。是个原语，不是判据。
    has_kana(s)       有没有假名。**单向可靠**：带假名 ⇒ 一定不是中文。
    is_chinese_text(s, src)
                      「这段文本是中文吗」—— 唯一该被当判据用的那个。

🔴 为什么是「删掉这个名字」而不是「写条注释提醒」：
   从另外六门拷过来的脚本必然写着 `dbtool.has_han(...)`。留着这个名字，
...
⚠️ 纯汉字串在字形上**分不出中日**（`漢字` 既是日文也是中文）⇒ `is_chinese_text`
...
#
# 🔴 2026-08-31 重列。七月版只追 6 列，德语的一等字段全在闸外面（见文件头）。
#    判据同 fr/it/pt：**凡是承载值的列全部进来**，只有 id / word / word_norm / is_lemma
#    四个身份列不进（它们 NOT NULL，非空计数恒等于总行数，计了也是常数）。
#
# ⚠️ **de 是单读音语言**：只有 `ipa` 一列、`ipa_src` 一个来源列。
#    pt 那版是双读音四列，照抄会追到不存在的列（快照会跳过，但闸就守不住任何东西）。
#    实测依据（阶段 -1）：德语版 105.9 万条音标里带奥/瑞地区标记的只有 1,419 条＝0.13%
#    ⇒ 不设 `ipa_at`/`ipa_ch`（`[[es-v3-structure-backfill]]`：
#    照搬别的语言结构前先量这门语言有没有那个病）。
TRACK = [
    # ══════════════════════════════════════════════════════════════
    # 🔴🔴 **条目可以写 `列` 或 `表.列`。** 2026-09-16 大改。
    #
    # 改之前这张表列了 23 个列名，而 `snapshot()` 只在 `dict` 上找 ——
    # **实际守住的只有 4 个**（kanji_grade / pos / level / freq_zipf）。
    # 另外 19 个包括日语**全部的一等字段**（kana / romaji / ipa / pitch_*），
    # 它们在 v3 里住在 `entry` 和 `pronunciation` 上，不在 `dict` 上。
    #
    # 为什么一直没人发现：`snapshot` 有一条「TRACK 里还不存在的列就跳过」，
    # 那是给「先写进清单、再由脚本建出来」留的口子 —— **合理的口子，
    # 但它让"名字写错了"和"还没建出来"长得一模一样**，于是 19 个死条目
    # 静静躺了整个项目（`[[criteria-narrower-than-you-think]]` 的极端形态：
    # 判据窄到只覆盖一张表，而清单看起来覆盖了十几个字段）。
    # ⇒ 现在 `_track_audit()` 会把「哪张表都找不到」的条目**大声报出来**。
    #
    # ── 读音层（日语的一等字段，相当于 de 的 gender/aux）──
    # 🔴 `kana` 是**属性不是主键分量**：3,951 条 entry 一条里就有 ≥2 个读音
    #    （`爺` = じい/じじ/じじい）。主键仍是 `(词形, 词性, etym_no, seq)`，见 JA_PLAN §二.1。
    'entry.kana', 'entry.kana_src', 'entry.kana_hist', 'entry.romaji',
    # 声调（东京式）。⚠️ **唯一可用来源是中文版**：日语版那份在抽取时就毁了
    #   （10,215 条里 10,212 条的 `other` 字面是 `"("`，重音位置全丢），见 JA_PLAN §一。
    'pronunciation.ipa', 'pronunciation.notation',
    'pronunciation.pitch_mark', 'pronunciation.pitch_pos',
    # 字种等级（常用/教育/人名用/表外）——「这个**字形**的属性」，不是逐义项标签
    'kanji_grade', 'entry.kanji_grade',
...
# 🔴 只收**真假名**，不收区间里的标点与符号。
#    2026-09-15 实测栽在这条上：`（古語・雅語）十一月` 是纯汉字的日语，却被判成"带假名"，
#    因为 `・`（KATAKANA MIDDLE DOT, U+30FB）落在 30A0–30FF 区间里。
#    后果不止一处 —— `has_kana` 还被 `is_chinese_text` 用着，
#    于是 `中文・英文` 这种带中点的**中文**会被判成"不是中文"。
#    ⚠️ 区间是**形式**，"是不是假名"是**内容**。按区间收就是拿形式当判据。
#    以下四类有意排除：・（中点）゠（双连字符）ー（长音符，本身不是假名）
#                      ゝゞヽヾ（叠字符号）
_KANA_RANGES = (
    (0x3041, 0x3096),      # 平假名 ぁ–ゖ
    (0x30A1, 0x30FA),      # 片假名 ァ–ヺ
...
def has_han_char(s):
    ...
    🔴 别拿它当判据用。日语的词头、释义、标签大量是纯汉字，它在这门语言上恒真。
...
def has_kana(s):
    """有没有假名。**这是个单向可靠的判据**：带假名 ⇒ 一定不是中文。

    反过来不成立 —— 不带假名不代表是中文（`漢字` 就是纯汉字的日文）。
    """
    return any(any(a <= ord(c) <= b for a, b in _KANA_RANGES) for c in s or "")


# 认得出「这段文本来自哪一版」的来源标记。判据用**来源**不用字形 ——
# `[[criteria-from-meaning-not-form]]`：纯汉字串在字形上分不出中日。
_ZH_SRC = {"zh-edition", "zh-wiktionary", "ecdict", "model", "manual"}
_JA_SRC = {"ja-edition", "kk-ja", "en-edition"}
...
def is_chinese_text(s, src=None):
    ...
    if src in _JA_SRC:
        return False
    raise ValueError(
        "纯汉字文本分不出中日，必须给 src 才能判：%r（已知中文源 %s／日语源 %s）"
        % (t[:30], sorted(_ZH_SRC), sorted(_JA_SRC)))
...
# 判据是**计划表里那一行**，不是手工开关：
#   `docs/JA_PLAN.md` 里出现 `# ✅ ja 完结` 才算完结。
...
PLAN_DOC = paths.ROOT / "docs" / "JA_PLAN.md"
...
def _language_is_done():
    ...
    删数据的默认值必须偏保守。"""
    try:
        return bool(re.search(r"^#+\s*✅\s*ja\s*完结", PLAN_DOC.read_text("utf-8"), re.M))
    except OSError:
        return False
