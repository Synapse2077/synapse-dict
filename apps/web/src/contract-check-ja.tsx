...
const CHECKS: Check[] = [
  ...
  // ── 活用表的表结构（2026-09-19）。用户就是在这一页上看见错的，闸必须也站在这一页上 ──
  // 🔴 数据层的 I1–I7 问的是「库里对不对」，这四条问的是「**读者看到什么**」。
  //    `[[correct-steps-can-compose-a-hole]]`：闸至少要有一条是读者口径。
  {
    word: '食べる',
    name: '🔴 活用表不再把源头的错标签原样印出来（外审就是在这一页上挑出来的）',
    hit: (t) => {
      const bad = [
        ['敬体命令形', '食べましょう 是意志形不是命令形（源头把意志形组标成了 imperative）'],
        ['敬体条件形', '食べさせられます 是使役被动不是条件形（源头把使役被动组标成了 conditional）'],
        ['愿望敬体', '食べています 是进行体不是愿望形（源头把进行体组标成了 desiderative）'],
      ].filter(([lab]) => t.includes(lab as string));
      return bad.length ? bad.map(([, why]) => why).join('；') : null;
    },
  },
  {
    word: '食べる',
    name: '🔴 最常用的普通体在页面上（源头报错标记曾把这几列整批丢掉）',
    hit: (t) => {
      // 用户点开活用表想查的就是这些。修之前 12 个查了 12 个全缺。
      const want = ['食べない', '食べた', '食べよう', '食べろ', '食べれば',
                    '食べたら', '食べたい', '食べている', '食べられる', '食べさせる'];
      const miss = want.filter((w) => !t.includes(w));
      return miss.length ? `页面上没有：${miss.join('、')}` : null;
    },
  },
  {
    word: '食べる',
    name: '🔴 敬体的现在与过去在页面上分得开（曾两行同印「敬体」）',
    hit: (t) => (t.includes('敬体过去') ? null
      : '`食べました` 没有印成「敬体过去」—— 与 `食べます` 同印「敬体」，读者无法区分'),
  },
  {
    word: '食べる',
    name: '文語形与现代活用分得开，ら抜き也标出来了',
    hit: (t) => {
      if (!t.includes('文語')) return '文語形没有标记 —— 混在现代活用里印，读者会以为现代日语能用';
      if (!t.includes('（ら抜き）')) return '`食べれます` 没标 ら抜き —— 与规范可能形并列，像两套都规范';
      return null;
    },
  },
  // ── 词源正文（2026-09-20）──
