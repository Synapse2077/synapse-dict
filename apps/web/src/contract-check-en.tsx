...
const CHECKS: Check[] = [
  ...
  // ───────────────── G. 词源分块 → **已挪到 `contract-check-etym.tsx`** ──────
  // 2026-09-12 先在这里写了 G1/G2，当天做另外五门时发现判据与语种无关，
  // 抄六份必然漂移（de 那 16 个孤儿类名、五门各抄一份的搭配 JSX 都是现成的账）。
  // ⇒ 合并成一份跨六门的闸：`apps/web/src/contract-check-etym.tsx`
  //   （3 条断言 × 5 种变异 × 252 个词，含 en）。
  // ⚠️ **这里不留残句**：一条不再生效的断言留着比删掉更坏 —— 下一个人会以为 en 有人守。
];
