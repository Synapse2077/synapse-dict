#!/usr/bin/env python3
"""kaikki 法语 dump 的正确读法 —— fr 的唯一入口。2026-08-01。

fr 独立文件，不 import 其他语种（见 multilang-decoupling-essence 铁律）。
...
KK = HERE / "kaikki.org-dictionary-French.jsonl"
...
#
# · **法语版用 `\…\` 作定界符**（不是 `/…/`），2026-08-01 曾因此把整个 fr 版读成空。
# · **英文版法语的 `forms` 有 93% 带 ipa** —— 这是 fr 变形层音标 76.9% 覆盖的来源，
#   五语种里独此一家（es/it/de/pt 的 forms 带 ipa 全是 0）。
# · 法语版是全项目最大的音标富矿：法语词 171 万带音标，另有意语 68 万 / 德语 50 万 /
#   葡语 29 万 / 西语 19 万 —— 它是**多语种词典**，不是只有法语。
# ═══════════════════════════════════════════════════════════════════════

EDITION = HERE / "fr-edition-extract.jsonl.gz"      # 语言版 dump（gz）
