...
7. 若该词是**形容词**，返回 "feminine" 字段给阴性单数形（如 grand→grande、heureux→heureuse、beau→belle、blanc→blanche）；阴阳同形(如 rouge)或不变则不给。
8. 返回 "ipa" 字段：该词音位式国际音标，法语约定——鼻元音 ɑ̃/ɛ̃/ɔ̃/œ̃、小舌音 ʁ、词尾哑辅音不发音、不标联诵、音节点 . 分隔、两侧加斜杠（如 livre /livʁ/、manger /mɑ̃.ʒe/、beaucoup /bo.ku/）。缩写/无读音的省略 ipa。
9. 返回 "col" 字段：该词最常用搭配或固定短语 1-3 条，形如 "eau potable 饮用水"（法语在前、中文在后，空格分隔）。只给搭配/短语，不要整句例句。无则 []。
10. 若你强烈认为缺了常见义项，在 flag 写"疑缺义:<简述>"，但不要为此往 zh 加义项（zh 数量必须严格等于 senses）。
11. 生僻/拿不准/可能歧义的词，flag 写简短原因；有把握则 flag=null。
12. 返回 "level" 字段：该词的 CEFR 难度等级，取值 A1/A2/B1/B2/C1/C2 之一（A1 最基础常用、C2 最生僻高阶）；按该法语词的实际使用频率与掌握难度判断，务必每词都给。
13. 严格输出 JSON，键与输入一致，格式 {"1":{"zh":[...],"aux":"avoir","gender":"f","plural":"...","feminine":"...","ipa":"/.../","col":["..."],"level":"B1","flag":null},...}（aux/gender/plural/feminine/ipa/col 无则省略，level 必给），不要多余文字。"""
...
def merge():
    ...
    for fp in sorted(glob.glob(str(OUT_DIR / "chunk_*.json"))):
        ...
        for rid, o in data.items():
            ...
            row = conn.execute(
                "SELECT pos, aux, gender, plural, feminine, ipa FROM dict WHERE id=?", (rid,)
            ).fetchone()
            if not row:
                continue
            pos, cur_aux, cur_gender, cur_plural, cur_fem, cur_ipa = row
    ...
    print(f"写回 {n} 词，flag {nf}；补缺 aux {n_aux}/gender {n_gender}/plural {n_plural}/"
          f"feminine {n_fem}/ipa {n_ipa}；搭配 {nc}；CEFR {n_lvl}")
