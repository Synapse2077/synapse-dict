#!/usr/bin/env python3
"""阶段 2b：变形层归位 —— `dict.infl`/`exchange` 两列字符串 → `inflection` 表。2026-08-22。

═══ 现状 ═══
    dict.infl      'livrer 的 命令式第二人称单数'      299,288 行，多条用 \\n 拼在一起
    dict.exchange  '0:livrer'                       同上
一个词形的多条变形关系挤在一个字符串里，查不了、也挂不住 entry。
`SCHEMA` §10 定的落点是 `inflection.entry_id` —— 变形归到**词条**上，不是词形上。

═══ 与阶段 2a 的关系（这一步为什么必须在 2a 之后）═══
`build.py:464` 是 `fo = s.get("form_of") or s.get("alt_of")` —— 两者一起当变形。
2a 已经把 `alt_of` 移回词条层了，所以 `infl` 列里那些
「et 的 变位形式」（`&` 是 `et` 的**缩写**，不是变位形式）**不该进变形层**。
**本步只收 `form_of`。**
...
═══ 复刻的是哪条规则（读自 `fr/pipeline/build.py:461-480, 595-596`）═══
