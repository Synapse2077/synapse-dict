#!/usr/bin/env python3
"""预计算短前缀的搜索结果（法语）—— 干掉搜索下拉的 265ms 尖峰。2026-08-28（阶段 9）。

═══ 缺陷（每次按键都撞到）═══
`search()` 是搜索下拉与划词的路径，**用户每敲一个字符就跑一次**。实测（进程内，非 HTTP）：

    1 字符前缀   中位 134.8 ms   最慢 **264.7 ms**（`d`）
    2 字符前缀   中位  35.0 ms   最慢 129.7 ms（`de`）
    3 字符前缀   中位   5.6 ms   最慢  22.5 ms
    4 字符及以上  ≈ 1 ms

fr 库 2,086,292 行，是六个语种里最大的一个，所以也是最慢的一个。
es/it 在 2026-08-20 已经做过这件事（`[[search-prefix-precompute]]`），**fr 漏了** ——
阶段 8 验收时数据还没长到今天这个量，`[[query-perf-collation-traps]]`：
**性能问题要等数据长大才咬人**。

═══ 瓶颈是排序不是过滤（es/it 那轮实测，机制相同）═══
为了取 20 条，把 2–3 万条候选整个排一遍。`ORDER BY` 里的 `LENGTH(word)` 与
...
🔴 **`LIVE` 与 `french.ts:prefixQuery` 逐字相同，一行都不 import it 那份。**
   两边的 `ORDER BY` 并不一样 —— it 是**两级**精确匹配（先 `word = ?` 再
   `lower(word) = lower(?)`），fr 只有一级。抄过来会**静默改掉排序**，
   而两边都不会报错（`[[fix-regression-and-gate]]`：判据只许一份，
   这里的"一份"是 `french.ts`，本文件是它的副本，改一边必须改两边）。

═══ 缓存未命中 = 正确回退，不是错误 ═══
键就是调用方传进来的原字符串。查不到就走实时查询 —— 结果一样，只是慢一点。
🔴 这条性质是有意设计的：SQLite 的 `lower()` **不认非 ASCII**（`lower('É')`='É'），
   而法语词头大量带重音符。与其去猜大小写归一规则，不如让键严格等于原串、猜不中就回退。
...
# 🔴 与 `packages/dict-core/src/french.ts` 的 `prefixQuery` **逐字相同**。
LIVE = """
SELECT id FROM dict
