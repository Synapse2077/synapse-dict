#!/usr/bin/env python3
"""fr 服务级质检 —— 全部走 2.1pro（量不大，不用 lite 路由）。

三个子命令：
  --fill-ipa    给 ipa 为空的 lemma（1530 条）补法语 IPA，直接写库。
...
IPA_SYS = """你是法语语音专家。我给你一批词（可能含外来词、地名、缩写、古拼写等），逐个给标准法语 IPA（维基/Larousse 式）。
约定：小舌音 ʁ；鼻元音 ɑ̃/ɛ̃/ɔ̃/œ̃；哑音结尾不发（词尾 -s/-t/-d/-x 一般不读）；不标重音；两侧加斜杠，例 /apʁɛ/。
缩写、纯数字序号、无单词读音的 → 该项返回 null。
严格输出 JSON，键与输入序号一致：{"1":"/.../","2":null,...}，无多余文字。"""

QA_SYS = """你是资深法语→简体中文词典审校专家。我给你一批已翻词条，每条含：词形 w、词性 pos、英文 gloss（消歧参考，可能有误）、以及待审的中文译文数组 zh（每项对应一个义项）。
请**独立**用你的法语知识逐义项核对 zh 是否准确、地道、无错义/漏译/多译/词性错配。英文 gloss 只作参考，以法语实际含义为准。
对每个词返回：
