#!/usr/bin/env python3
"""es 工具的金标准测试 —— 把 2026-08-01 翻过的车逐条固化成断言。
...
跑：  python3 -m unittest test_tools -v      （在 es/ 目录下）
"""
...
class 解析(unittest.TestCase):
    ...
    def test_音位式与严式拼一起时只取音位式(self):
        """gracias 的 sounds.ipa 是 `/ˈɡɾaθjas/ [ˈɡɾa.θjas]` 整串。
        strip("/") 会把严式段一起带出来，导致该词永远比对不上。"""
        self.assertEqual(K.parse_ipa("/ˈɡɾaθjas/ [ˈɡɾa.θjas]"), "ˈɡɾaθjas")
