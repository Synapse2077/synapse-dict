#!/usr/bin/env python3
"""es 服务级质检 —— 全部走 2.1pro（量不大，不用 lite 路由）。

三个子命令：
  --fill-ipa    给 phonetic 为空的 lemma（25 条）补音位式 IPA，直接写库。
...
IPA_SYS = """你是西班牙语语音专家。我给你一批词（可能含外来词、地名、缩写、古拼写 ç 等），逐个给音位式 IPA。
...
QA_SYS = """你是资深西班牙语→简体中文词典审校专家。我给你一批已翻词条，每条含：词形 w、词性 pos、英文 gloss（消歧参考，可能有误）、以及待审的中文译文数组 zh（每项对应一个义项）。
请**独立**用你的西语知识逐义项核对 zh 是否准确、地道、无错义/漏译/多译/词性错配。英文 gloss 只作参考，以西语实际含义为准。
对每个词返回：
...
def fill_ipa():
    ...
    rows = conn.execute(
        "SELECT id, word FROM dict WHERE is_lemma=1 AND (phonetic IS NULL OR TRIM(phonetic)='')"
    ).fetchall()
    ...
    for m, res in zip(metas, results):
        ...
        for k, rid in enumerate(m, 1):
            ...
            if isinstance(ipa, str) and ipa.startswith("/"):
                conn.execute("UPDATE dict SET phonetic=? WHERE id=?", (ipa, rid))
                n += 1
