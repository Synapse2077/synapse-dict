#!/usr/bin/env python3
"""预计算短前缀的搜索结果（意语）—— 干掉搜索下拉的 490ms 尖峰。2026-08-20。

═══ 缺陷（每次按键都撞到）═══
    search 前缀   P50 5.2ms   P95 44ms   P99 184ms   max 490ms  ← 比西语还慢
...
意语库 1,498,956 行（比西语大 30%），瓶颈完全相同：为了取 20 条把候选整个排一遍。
⚠️ **它是独立实现，一行都不 import 西语那份** —— 两边 `prefixQuery` 的 `ORDER BY`
   并不一样（意语是**两级**精确匹配：先 `word = ?` 再 `lower(word) = lower(?)`），
   抄过来会静默改掉排序。SQL 逐字取自 `italian.ts:prefixQuery`。
...
# 与西语同样定在 3：1–2 字符降下来之后最慢的会变成 3 字符的高频前缀。
MAXLEN = 3
...
# 🔴 与 `italian.ts:prefixQuery` **逐字相同**（含那两级精确匹配）。
#    差一个字，预计算出来的顺序就与页面不一致，而两边都不会报错。
LIVE = """
SELECT id FROM dict
