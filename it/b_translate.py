...
11. 严格输出 JSON，键与输入一致，格式 {"1":{"zh":[...],"aux":"avere","gender":"f","plural":"...","ipa":"/.../","col":["..."],"flag":null},...}（无则省略），不要多余文字。"""
...
def merge():
    conn = sqlite3.connect(str(DB_PATH))
    n = nf = n_aux = n_gender = n_plural = n_ipa = nc = 0
    for fp in sorted(glob.glob(str(OUT_DIR / "chunk_*.json"))):
        ...
        for rid, o in data.items():
            ...
            sets, vals = ["translation=?", "flag=?", "collocation=?"], [tr, flag, collocation]
    ...
    conn.commit()
    conn.close()
    print(f"写回 {n} 词，flag {nf}；补缺 aux {n_aux}/gender {n_gender}/plural {n_plural}/ipa {n_ipa}；搭配 {nc}")
