// 搜索路径：**结果一致性 + 延迟**（意语）。2026-08-20。
// ⚠️ 与西语那份是两个独立实现：意语的 ORDER BY 是**两级**精确匹配。
//
// `search()` 每敲一个字符跑一次。1 字符前缀原本最慢 490ms（为取 20 条把 2–3 万条
...
import { ItalianDictService } from '../../packages/dict-core/src/italian.ts';

const ROOT = new URL('../../', import.meta.url).pathname;
const DB = `${ROOT}data/db/synapse-dict-it.sqlite`;
const d = new ItalianDictService(DB, `${ROOT}data/tts/it`);
const raw = new DatabaseSync(DB, { readOnly: true });

// 实时查询（与 italian.ts:prefixQuery 内层同一份 SQL）
const live = raw.prepare(`
  SELECT id FROM dict
  WHERE word LIKE ? COLLATE NOCASE OR word_norm LIKE ? COLLATE NOCASE
  ORDER BY CASE WHEN word = ? THEN 0 WHEN lower(word)=lower(?) THEN 1 ELSE 2 END,
...
for (const p of prefixes) {
  ...
  const want = (live.all(`${k}%`, `${k}%`, k, k, 20) as Array<{ id: number }>).map((r) => r.id);
