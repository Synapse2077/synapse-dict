...
it 是**桥建在本语言释义层还不存在的时候**：`pipeline/ingest_examples.py` 的文件头
...
⇒ 判据：`example.src_gloss` 存着这条例句在源里挂的那条**本语言释义原文**，
  按 **(词形, 本语言释义原文)** 精确匹配 `sense_gloss.lang='it'`。
...
def main():
    ...
    with dbtool.session("it-relink-examples", expect={}) as s:
