...
function mapEntry(row: EsRow): SpanishEntry {
  ...
  return {
    ...
    etymologyTexts: {},               // 另表，由 getEntry 填（同上）
    etymologyEditions: [],            // 同上
...
export class SpanishDictService {
  ...
  constructor(databasePath: string, ttsDir?: string) {
    ...
    this.collocDetailQuery = this.db.prepare(`
      SELECT c.text AS text, d.word AS owner,
             ...
             (SELECT text FROM collocation_gloss
               WHERE collocation_id = c.id AND lang = 'es') AS srcText
  ...
  getEntry(word: string): SpanishEntry | null {
    ...
    entry.etymologyEditions = this.etymEditions;
    entry.etymologyTexts = Object.fromEntries(
