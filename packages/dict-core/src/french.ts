...
export type FrenchSearchVia = { word: string; kind: 'collocation'; src: string | null };

export type FrenchCollocation = {
...
export class FrenchDictService {
  ...
  constructor(databasePath: string) {
    ...
    this.collocDetailQuery = this.db.prepare(`
      SELECT c.text AS text, d.word AS owner,
             ...
             (SELECT text FROM collocation_gloss
               WHERE collocation_id = c.id AND lang = 'fr') AS srcText
  ...
  search(query: string, limit = 20): FrenchSearchItem[] {
    ...
    const like = `${keyword}%`;
    const items: FrenchSearchItem[] = rows.map((r) => ({
      id: r.id,
      word: r.word,
      pos: r.pos,
  ...
  private mapEntry(row: FrRow, deep: boolean): FrenchEntry {
    ...
    const entry: FrenchEntry = {
      ...
      collocations: (this.colsQuery.all(row.id) as Array<{
        text: string; zh: string | null; src: string | null;
      }>).map((c) => ({ text: c.text, zh: c.zh, src: c.src })),
      inflections: infl,
